...
std::string Network::postDisplayString(Post* post) {
    ...
    std::string name = users_[authorID]->getName();
...
std::string Network::getPostsString(int profileId, int howMany) {
    ...
    if (userPosts.empty() || howMany <= 0) {
        return "";
    }
    
    int numPosts = howMany;
    if (userPosts.size() < howMany) {
        numPosts = userPosts.size();
    }
    
    std::string result = "";

    for (int i = 0; i < numPosts; i++) {
        int postIndex = userPosts.size() - 1 - i;  
        result += postDisplayString(userPosts[postIndex]);
...
int Network::readPosts(char* fname) {
    ...
    for (int i = 0; i < numPosts; i++) {
        ...
        std::string urlLine;
        std::getline(fileIn, urlLine);

        if (!urlLine.empty() && urlLine[0] == '\t') {
            urlLine = urlLine.substr(1);
        }
        
        bool isLinkPost = !urlLine.empty();
        
        Post* post;
        if (isLinkPost)
            post = new LinkPost(profileId, authorId, message, likes, urlLine);
        else
            post = new Post(profileId, authorId, message, likes);

        post->setMessageId(messageId);
        
        if (profileId >= 0 && profileId < (int)users_.size()) {
...
int Network::writePosts(char* fname) {
    ...
    std::sort(allPosts.begin(), allPosts.end(),
              [](Post* a, Post* b) { return a->getMessageId() < b->getMessageId(); });
