...
public:
    ...
    // pre: post is not nullptr
    // post: adds post to the posts_ vector for the appropriate profileId; posts_[profileId] grows by one
    void addPost(Post* post);

    // pre: id is a valid user ID (0 <= id < users_.size())
    // post: returns a vector of all posts for user id, or empty vector if none exist
    std::vector<Post*> getPosts(int id);

    // pre: post is not nullptr
    // post: returns a string representation of the post suitable for display
    std::string postDisplayString(Post* post);

    // pre: profileId is a valid user ID; howMany >= 0
    // post: returns a string containing up to howMany most recent posts for user profileId, or fewer if user has fewer posts; returns empty string if no posts
    std::string getPostsString(int profileId, int howMany);

    // pre: fname points to a valid file containing post data in correct format
    // post: posts read from file and added to posts_; returns 0 on success, -1 on error
    int readPosts(char* fname);
    
    // pre: fname points to a writable file path
    // post: writes all posts to file sorted by messageId; returns 0 on success, -1 on error
